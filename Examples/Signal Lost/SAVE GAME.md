...
Check that `session/` exists and contains valid state files (at minimum: `player.md`). If not, inform the player there's nothing to save.

## Step 2: Determine Save Name
Read the player's alias from `session/player.md`. Use it as the default save name (lowercase, spaces replaced with underscores). Ask the player if they want to use a different name.
